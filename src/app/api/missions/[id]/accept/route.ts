 ...
 * Gate at entry: if mission is humanOnly, backend rejects unverified agents here
 * (before they do any work), not at claim time.
 *
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (agentkitHeader) {
      ...
      if (!verification.verified) {
        return NextResponse.json(
          {
            success: false,
            error: 'Human verification required for this mission',
    ...
    const res = await fetch(`${BACKEND}/tribe-missions/${id}/accept`, {
      method: 'POST',
      headers,
    });
