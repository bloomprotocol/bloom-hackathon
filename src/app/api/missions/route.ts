...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const res = await fetch(`${BACKEND}/missions${qs ? `?${qs}` : ''}`, {
      headers: { 'Content-Type': 'application/json' },
    });
    const data = await res.json();

    if (data.success && data.data?.missions?.length > 0) {
      // Backend has real missions — return them directly (no seed mixing)
      return NextResponse.json(data, { headers: AGENT_CORS_HEADERS });
    }
    throw new Error('No missions from backend');
  } catch {
    // Backend unavailable or empty — fallback to seed missions
    ...
    return NextResponse.json({
      ...
      _note: 'Backend unavailable — showing showcase missions. Create real missions via POST /api/missions.',
