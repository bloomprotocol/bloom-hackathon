 ...
 *
 * Backend also verifies: belt-and-suspenders approach.
 *
  ...
  try {
    ...
    if (!question || typeof question !== 'string' || question.length < 10) {
      return NextResponse.json(
        { error: 'Please share more about your situation (min 10 characters)' },
        { status: 400 },
      );
    }
    ...
    if (hasAgentkitHeader) {
      ...
      logger.info('[Sanctuary Council] Agent verified', {
        address: verification.address,
        humanId: verification.humanId,
      });
    }

    // Forward to backend (which also runs AgentKit verification)
    try {
      const response = await fetch(`${BACKEND_API_URL}/sanctuary/council`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          type: ['business', 'creative', 'personal'].includes(type)
            ? type
            : 'business',
        }),
      });
        ...
        return NextResponse.json(
          {
            error: 'Human verification required',
            message:
              'Your agent must be registered in AgentBook with World ID. Run: npx @worldcoin/agentkit-cli register <your-wallet-address>',
            docs: 'https://docs.world.org/agents/agent-kit/integrate',
          },
          { status: 403 },
        );
      ...
      logger.warn('[Sanctuary Council] Backend unavailable, returning demo');
    }

    // Demo response (backend not available)
    return NextResponse.json({
      status: 'demo',
      message:
        'The Council is listening. In production, agents verify via AgentKit (proof of human, free access).',
      playbook: 'https://bloomprotocol.ai/paste-blocks/the-council-v1.md',
      note: 'Paste the playbook URL into your agent to run The Council locally. Your conversation stays on your machine.',
    });
  } catch (error) {
    logger.error('[Sanctuary Council] Error', { error });
    return NextResponse.json(
